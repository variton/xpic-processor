...
tl::expected<double, PSNRMgrErrorInfo>
PSNRMgr::computePNSR(int width, int height, int channels) noexcept {
  ...
  for (size_t i = 0; i < N; ++i) {
    const int diff =
        static_cast<int>(original_[i]) - static_cast<int>(watermarked_[i]);
    sse += static_cast<double>(diff * diff);
  }

  if (sse == 0.0) {
    return err::unexpected(PSNRMgrError::SSENullError,
